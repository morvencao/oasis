            ...
            steps {
                script {
                    try{
                        ...
                        oc --kubeconfig /root/.kube/svc-cluster.kubeconfig port-forward -n maestro svc/maestro 8000:8000 &
                        oc --kubeconfig /root/.kube/svc-cluster.kubeconfig port-forward -n maestro svc/maestro-grpc 8090:8090 &
                        echo "==== clone maestro ===="
                        git clone -b "${params.GIT_BRANCH}" https://github.com/openshift-online/maestro.git maestro/
                        echo "==== enter maestro directory ===="
                        cd maestro
                        echo "==== run e2e tests ===="
                        ginkgo -v --label-filter="!(e2e-tests-spec-resync-reconnect||e2e-tests-status-resync-reconnect||e2e-tests-spec-resync-restart||e2e-tests-status-resync-restart)" --output-dir=./test/e2e/report --json-report=report.json --junit-report=report.xml ./test/e2e/pkg -- -api-server=http://localhost:8000 -grpc-server=localhost:8090 -server-kubeconfig="/root/.kube/svc-cluster.kubeconfig" -consumer-name=aro-hcp-acmqe-eastus-mgmt-cluster -agent-kubeconfig="/root/.kube/mgmt-cluster.kubeconfig" -agent-namespace=maestro
                        """
                    }
                    catch (e){
